...
(function(){
  ...
  theme.Variants = (function() {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _onSelectChange: function({srcElement}) {
        ...
        this._updateVariantTabContent(variant);
        this._updateUnitPrice(variant);
        this._updateSKU(variant);
        this._updateDeliveryTime(variant);
      ...
      _updateVariantTabContent: function(variant) {
  
        this.container.dispatchEvent(new CustomEvent('variantTabChange', {
          detail: { 
            variant: variant
          }
        }));
      ...
      _updateDeliveryTime: function(variant) {
        this.container.dispatchEvent(new CustomEvent('variantDeliveryTimeChange', {
          detail: {
            variant: variant
          }
        }));
      ...
      _updateHistoryState: function(variant) {
        ...
        window.history.replaceState({path: newurl}, '', newurl);
         function showQuantityAlertForVariant(variantId) {
    const alerts = document.querySelectorAll('.quantity_alert_content');

    alerts.forEach(function(alert) {
      const currentId = alert.getAttribute('data-variant-id');
      if (currentId === String(variantId)) {
  ...
  // Example usage:
  const variantId = variant.id; // your actual variant ID
  console.log('test' + variantId);
  showQuantityAlertForVariant(variantId);
      },
  ...
  theme.CartForm = (function() {
    ...
    function CartForm(form) {
      ...
      this.finalTotal = form.querySelector(selectors.finalTotal);
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function(text) {
        ...
        var final_total = items.dataset.cartSubtotal;
        ...
        this.finalTotal.innerHTML = theme.Currency.formatMoney(final_total, theme.settings.moneyFormat);
  ...
  // Function to initialize ProductForm for each form in the product grid
  theme.initProductGridForms = function() {
    var productForms = document.querySelectorAll('.grid-product .product-single__form');
    productForms.forEach(function(form) {
      new theme.AjaxProduct(form);
    });
  };
  
  // Initialize product forms when document is ready
  document.addEventListener('DOMContentLoaded', function() {
    theme.initProductGridForms();
  });
  ...
  theme.headerNav = (function() {
    ...
    function scrollHandler() {
      if (window.scrollY > 250) {
        ...
        document.querySelector(".site-header__search-container_mobile").classList.add(config.stickyClass);
        ...
        siteHeader.classList.remove(config.stickyClass);
         document.querySelector(".site-header__search-container_mobile").classList.remove(config.stickyClass);
  ...
  theme.SlideshowSection = (function() {
    ...
    SlideshowSection.prototype = Object.assign({}, SlideshowSection.prototype, {
      ...
      onSlideChange: function() {
        if (this.slideshow.classList.contains('initial-render')) {
          this.slideshow.classList.remove('initial-render');
        }
      },
  ...
  //       this.daysPlaceholder.innerHTML = intervals.days.toString().padStart(2, '0');
  //       this.hoursPlaceholder.innerHTML = intervals.hours.toString().padStart(2, '0');
  //       this.minutesPlaceholder.innerHTML = intervals.minutes.toString().padStart(2, '0');
  //       this.secondsPlaceholder.innerHTML = intervals.seconds.toString().padStart(2, '0');
  //     } else {
  ...
  //   initialize() {
  //     this.arrowButton = this.querySelector('.countdown-arrow');
  //     this.countdownWrapper = this.querySelector('.countdown-wrapper');

  //     // Check sessionStorage on load and set visibility
  ...
  //       });
  //     }
  //     document.addEventListener('countdown:complete', () => this.handleCountdownComplete());
  //   }

  //   toggleCountdown() {
  //     if (this.countdownWrapper) {
  //       this.countdownWrapper.classList.toggle('closed');
  //       this.arrowButton.classList.toggle('closed');

  //       // Update session storage based on the state of the countdown
  //       const isClosed = this.countdownWrapper.classList.contains('closed');
  //       sessionStorage.setItem('countdownHide', isClosed ? 'true' : 'false');
  //     }
  //   }

  //   // Hide countdown on completion and save state to sessionStorage
  //   handleCountdownComplete() {
  //     if (this.countdownWrapper) {
  //       this.countdownWrapper.classList.add('hide-countdown-wrapper');
  //     }
  //     if (this.arrowButton) {
  //       this.arrowButton.classList.add('hide-countdown-wrapper');
  //     }
  //     document.body.classList.add('countdown-complete');
  //     sessionStorage.setItem('countdownHide', 'true');
  //   }

  //   // Check session storage to determine if the countdown should be visible
  //   checkVisibility() {
  //     const countdownHide = sessionStorage.getItem('countdownHide');
  //     if (countdownHide === 'true') {
  //       if (this.countdownWrapper) {
  //         this.countdownWrapper.classList.add('closed');
  //       }
  //       if (this.arrowButton) {
  //         this.arrowButton.classList.add('closed');
  //       }
  //     }

  //     /** Start Customization of Better Sell Online GmbH - listen to the event when time is complete **/
  //     document.addEventListener('countdown:complete', () => this.handleCountdownComplete());
  //     /** End Customization of Better Sell Online GmbH **/
  //   }
  // }
      ...
      colorSwatchHovering: function() {
        ...
        this.container.querySelectorAll(selectors.colorSwatch).forEach(swatch => {
          swatch.addEventListener('mouseenter', () => {
            ...
            var id = swatch.dataset.variantId;
            var image = swatch.dataset.variantImage;
            var el = swatch.closest(".grid__item").querySelector('.grid-product__color-image--' + id);
           // console.log(el);
          ...
          swatch.addEventListener('mouseleave', () => {
            ...
            const allColorImages = this.container.querySelectorAll('.grid-product__color-image');
            allColorImages.forEach(img => img.classList.remove('is-active'));
          });
        });
      },
  ...
  theme.Product = (function() {
    ...
    function Product(container) {
      ...
      this.selectors = {
        ...
        percentageContainer: '.percentage-container.percentage-text-pdp',
        unitWrapper: '[data-unit-price-wrapper]',
        unitPrice: '[data-unit-price]',
        unitPriceBaseUnit: '[data-unit-base]',
        sku: '[data-sku]:not(.ruk_rating_snippet)',
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      cacheElements: function() {
        this.cache = {
          ...
          percentageContainern: this.container.querySelector(this.selectors.percentageContainer),
      ...
      initVariants: function() {
        ...
        this.container.on('variantTabChange' + this.settings.namespace, this.updateVariantTabContent.bind(this));
        this.container.on('variantPriceChange' + this.settings.namespace, this.updatePrice.bind(this));
        this.container.on('variantUnitPriceChange' + this.settings.namespace, this.updateUnitPrice.bind(this));
        this.container.on('variantDeliveryTimeChange' + this.settings.namespace, this.updateDeliveryTime.bind(this));
      ...
      updateCartButton: function(evt) {
        ...
        var saleLabel = document.querySelector(".product-section .grid-product__tag--s");
      ...
      updatePrice: function(evt) {
        ...
        if (variant) {
          ...
          if (variant.compare_at_price > variant.price) {
            ...
            if(!evt.srcElement.classList.contains('is-modal')) {
              if (this.cache.percentageContainern) {
                this.cache.percentageContainern.classList.remove(classes.hidden);
              }
            }
            ...
            if (!!this.cache.percentageContainern) {
              this.cache.percentageContainern.classList.add(classes.hidden);
            }
      ...
      updateVariantTabContent: function(evt) {
        var variant = evt.detail.variant;
        if (evt.target.classList.contains("is-modal")) {
          this.rerenderSection(this.container.dataset.sectionId, this.container.dataset.productHandle, variant.id, true);
        } else {
          this.rerenderSection(this.container.dataset.sectionId, this.container.dataset.productHandle, variant.id);
        }
        const tabsVariant = document.querySelectorAll('[data-tab-variant]');
        const tabsLayoutVariant = document.querySelectorAll("[data-tab-layout-variant]");
        if (tabsVariant.length > 0 ) {
          tabsVariant.forEach(tab => {
            tab.style.display = 'none';
          });
          document.querySelectorAll('#Variant-' + variant.id).forEach(variantTab => {
              const richTextField = variantTab.querySelector('.metafield-rich_text_field');
              const customTab = variantTab.closest('.custom_tab');
              if (richTextField && richTextField.innerHTML.trim() == '') {
                customTab.style.display = 'none';
              }else{
                variantTab.style.display = '';
                customTab.style.display = '';
              }
          });
        }
        if(tabsLayoutVariant.length > 0){
          document.querySelectorAll('[data-tab-layout-variant]').forEach(tab => {
            tab.style.display = 'none';
          });
          document.querySelectorAll('#tab-' + variant.id).forEach(variantTab => {
              const richTextField = variantTab.querySelector('.metafield-rich_text_field');
              const customTabId = variantTab.closest('.tab_content_main')?.getAttribute('id');
              const tab_ = document.querySelector('.variant-tabs-nav a[href="#' + customTabId + '"]')?.closest('li');
              if (richTextField && richTextField.innerHTML.trim() == '') {
        ...
        // document.querySelectorAll('[data-tab-variant]').forEach(tab =>{
        //   tab.style.display = 'none';
        // });
        // document.querySelectorAll('#Variant-'+variant.id).forEach(variantTab =>{
        //   variantTab.style.display = '';
      ...
      rerenderSection: function(sectionId, productHandle, variantId, isModal = false) {
        let section = document.querySelector(`#shopify-section-${sectionId}`);

        let newUrl = new URL(document.location);
        newUrl.pathname = '/products/' + productHandle;
        ...
        let currentUsedVariant = null;
        if (isModal) {
          section = document.querySelector(`#QuickShopHolder-${productHandle}`);
          currentUsedVariant = this.variantsObject.find(x => x.id === variantId);
        } 
        ...
        .then(data => {
          if (section) {
            let sectionSavings = section.querySelector('.product-block.product-block--price');
            if (sectionSavings) {
              if (!isModal) {
                let template = document.createElement('template');
                template.innerHTML = data.trim();
                let savings = template.content.firstChild.querySelector('.product-block.product-block--price');
                let firstChild = savings;
                sectionSavings.replaceWith(firstChild);
              } else {
                let innerSavings = sectionSavings.querySelector('.flex.justify-start');
                if (innerSavings) {
                  let percentageTainer = innerSavings.querySelector('.percentage-container.percentage-text-pdp');
                  let comparePRICING = innerSavings.querySelector('[data-product-price-wrap]');
                  // if there is a percentage container, update it
                  if (percentageTainer && comparePRICING) {
                    let totalPercentSavings = (currentUsedVariant.compare_at_price - currentUsedVariant.price) / currentUsedVariant.compare_at_price;
                    percentageTainer.innerHTML = '-' + Math.round(totalPercentSavings * 100) + '%';
                    if (!comparePRICING.classList.contains('hide')) {
                      percentageTainer.classList.remove('hide');
                    }
                  }
                }
              }
            } else {
              console.error("No product-block--price element found");
            }
          }
        }).catch(function(err) {
            console.error(err);
      ...
      updateDeliveryTime: function(evt) {
        const variant  = evt.detail.variant;
        let fetch_url = this.container.dataset.productUrl + '?variant=' + variant.id + '&section_id=' + this.container.dataset.sectionId;
        let time_selector = this.selectors.deliveryTime;
        let current_delivery_time_node = this.container.querySelector(time_selector);
        fetch(fetch_url)
        .then(function(response) {
          return response.text();
        })
        .then(function(html) {
          let new_delivery_time_node = document.createElement('div');
          new_delivery_time_node.innerHTML = html.trim();
          new_delivery_time_node = new_delivery_time_node.querySelector(time_selector);
          if (new_delivery_time_node) {
            current_delivery_time_node.innerHTML = new_delivery_time_node.innerHTML
          }
        });
      },
      ...
      openModalProduct: function() {
        ...
        if (!this.settings.modalInit) {
          ...
          fetch(url).then(function(response) {
            ...
            this.container.querySelectorAll('script.initProductThumbnailSlider').forEach(script => {
  ...
  // START Customization by Better Sell Online GmbH (BSO) - create scroll animation for an anchor using scrollTo class
  theme.scrollToAnchor = function() {
    ...
    anchorLinks.forEach(anchorLink => {
      anchorLink.addEventListener('click', function(e) {
        ...
        if (targetElement) {
          const offsetTop = targetElement.getBoundingClientRect().top - 50;
          window.scrollTo({
            top: offsetTop,
            behavior: 'smooth'
          });
  ...
  // END Customization by Better Sell Online GmbH (BSO)
  ...
  DOMready(function(){
    ...
    // Customization by Better Sell Online GmbH (BSO) - add the scrollToAnchor to initialization
    theme.scrollToAnchor();
    theme.initGlobals();
    theme.initQuickShop();
    theme.rteInit();
